...
function mapTypeToCollection(type?: string, category?: string): string {
    ...
    // Topwear
    if (["shirt", "t-shirt", "tshirt", "top", "blouse", "sweatshirt", "sweater", "hoodie", "jacket"].includes(t)) return "topwear";
    if (["shirts", "t-shirts", "tops", "sweaters", "sweatshirts", "jackets"].includes(c)) return "topwear";

    // Bottomwear
    if (["jeans", "trouser", "trousers", "pants", "shorts", "skirt", "leggings"].includes(t)) return "bottomwear";
    if (["jeans", "trousers", "shorts", "skirts", "leggings"].includes(c)) return "bottomwear";

    // Footwear
    if (["shoe", "shoes", "sneaker", "sneakers", "boot", "boots", "heel", "sandals"].includes(t)) return "footwear";
    if (["footwear", "sneakers", "boots", "shoes", "sandals"].includes(c)) return "footwear";

    // Accessories
    if (["belt", "watch", "sunglasses", "cap", "hat", "bag", "backpack", "jewelry", "scarf", "gloves"].includes(t)) return "accessories";
    if (["accessories", "belts", "watches", "sunglasses", "bags", "wallets", "jewelry", "scarves"].includes(c)) return "accessories";

    // Dresses
    if (["dress", "maxi dress", "party dress"].includes(t)) return "dresses";
    if (["dresses", "party dresses", "maxi dresses"].includes(c)) return "dresses";

    // Winter wear
    if (["winter wear", "sweater", "jacket", "coat"].includes(c) || ["parka", "coat"].includes(t)) return "winter wear";
            ...
            <div className="mx-auto max-w-7xl px-4">
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Save products you love and shop later.</p>
                    </div>
                    {items.length > 0 && (
                            ...
                            <div key={product.id} className="relative">
                                ...
                                <div className="absolute top-3 left-3">
                                    <Badge variant="secondary" className="capitalize">
                                        {mapTypeToCollection((product as any).type, product.category)}
                                    </Badge>
                                </div>
                                <div className="absolute top-3 right-3">
                                    <Button size="sm" variant="secondary" onClick={() => removeItem(product.id)}>Remove</Button>
                                </div>
                            </div>
                        ))}
                    </div>
