...
import { usePathSegments } from "@/services/pathname";
...
const FavoriteIcon = ({ isFavourite, toggleFavourite }) => (
  <Heart
    ...
    className={`absolute cursor-pointer top-2 right-2 bg-transparent rounded-full p-0 transition-all ease-in-out ${
      isFavourite
        ? "text-accent-default fill-accent-default"
...
const ProductDetails = ({ product, formattedPath }) => (
  <Link href={`/${formattedPath}/${product.name}`} passHref>
    <div className="flex items-center gap-2 justify-between">
      <CardTitle className="text-description font-description truncate group-hover:underline underline-offset-2">
        {product.name}
      </CardTitle>
    </div>
...
const ProductPrice = ({ product }) => (
  <motion.div className="flex items-center gap-2 my-2">
    ...
    {product.discounted_price ? (
      <motion.div className="flex items-center gap-2">
        <span className="text-xs line-through text-muted-foreground/80">
          ${product.original_price}
        </span>
        <span className="text-small font-subheading text-primary-default">
          ${product.discounted_price}
        </span>
      </motion.div>
    ) : (
      <span className="text-small text-primary-default font-heading">
        ${product.original_price}
      </span>
    )}
  </motion.div>
  ...
  const handleQuantity = (id, value) => {
    setCart((prevCart) =>
      prevCart.map((cartItem) =>
        cartItem.id === id
          ? { ...cartItem, quantity: Math.max(min, value) }
          : cartItem
      )
    );
  };
      ...
      <Card className="w-full border-none h-fit">
        <CardHeader className="p-0 w-full overflow-hidden relative rounded-t-xs">
          ...
          <FavoriteIcon
            ...
            toggleFavourite={() => setIsFavourite((prev) => !prev)}
          />
        </CardHeader>
