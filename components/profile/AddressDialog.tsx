...
import { Navigation, Home, BriefcaseBusiness, MapPinHouse, CircleCheck, Loader2 } from "lucide-react";
    ...
    const detectLocation = async () => {
        ...
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                ...
                try {
                    ...
                    const state = states.find(s => s.name.toLowerCase() === stateName?.toLowerCase()) ||
                        states.find(s => stateName && (s.name.toLowerCase().includes(stateName.toLowerCase()) ||
                            stateName.toLowerCase().includes(s.name.toLowerCase())));

                    const detectedData = {
                        address1: [address.house_number, address.road || address.street].filter(Boolean).join(" "),
            ...
            (error) => {
                ...
                toast.error(errorMap[error.code] || "Failed to detect location", { id: toastId });
            },
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-xl p-0 overflow-hidden bg-background border-border shadow-2xl">
                ...
                <Form {...form}>
                    <form id="address-form" onSubmit={form.handleSubmit(onSubmit)} className="max-h-[70vh] overflow-y-auto px-6 py-4">
                            ...
                            <section className="space-y-4">
                                ...
                                <div className="grid gap-4">
                                            ...
                                            <FormItem>
                                                ...
                                                <FormControl>
                                                    <div className="flex gap-2">
                                                        ...
                                                        >
                                                            {isDetectingLocation ? <Loader2 className="size-4 animate-spin text-primary" /> : <Navigation className="size-4 text-primary fill-primary" />}
                                                            <span className="text-xs font-medium">{isDetectingLocation ? "Detecting..." : "Detect Location"}</span>
                                                        </Button>
                                                    </div>
                                                </FormControl>
                                    ...
                                    <div className="grid sm:grid-cols-2 gap-4">
                                        {[
                                            { name: "address2", label: "Address Line 2 (Optional)", placeholder: "Apt, Suite, Floor" },
                                            { name: "landmark", label: "Landmark (Optional)", placeholder: "E.g. Near City Mall" }
                                        ].map((f) => (
                                            <FormField
                                                key={f.name}
                                                control={form.control}
                                                ...
                                                render={({ field }) => (
                                                    <FormItem>
                                                        <FormLabel className="text-xs font-bold uppercase text-muted-foreground">{f.label}</FormLabel>
                                                        <FormControl><Input placeholder={f.placeholder} className="h-10 bg-muted/20" {...field} /></FormControl>
                                                        <FormMessage className="text-[10px]" />
                                                    </FormItem>
                                                )}
                                            />
                                                ...
                                                <FormItem>
                                                    ...
                                                    <FormControl>
                                                        <CountrySelect
                                                            ...
                                                            onChange={(val: string) => { field.onChange(val); form.setValue("state", ""); form.setValue("city", ""); }}
                                                        />
                                                    </FormControl>
                                                    <FormMessage className="text-[10px]" />
                                                </FormItem>
                                                ...
                                                <FormItem>
                                                    ...
                                                    <FormControl>
                                                        <StateSelect
                                                            ...
                                                            value={field.value}
                                                            className="h-10 bg-muted/20"
                                                            onChange={(val: string) => { field.onChange(val); form.setValue("city", ""); }}
                                                        />
                                                    </FormControl>
                                                    <FormMessage className="text-[10px]" />
                                                </FormItem>
                                        ...
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <FormItem>
                                                    ...
                                                    <FormControl><Input placeholder="Enter ZIP" className="h-10 bg-muted/20" {...field} /></FormControl>
                                                    <FormMessage className="text-[10px]" />
                                                </FormItem>
                                            )}
                                        />
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem className="space-y-3">
                                                ...
                                                <FormControl>
                                                    <RadioGroupPrimitive.Root onValueChange={field.onChange} value={field.value} className="flex flex-wrap gap-2">
                                                        {ADDRESS_TYPES.map((option) => (
                                                            <RadioGroupPrimitive.Item
                                                                ...
                                                                className={cn(
                                                                    ...
                                                                    "data-[state=checked]:border-primary data-[state=checked]:bg-primary border-transparent"
                                                                )}
                                                            >
                                                                <IconBadge variant={option.variant} size="sm">
                                                                    <option.icon className="size-3 transition-all ease-in-out" />
                                                                </IconBadge>
                                                                <span className="text-xs font-bold tracking-wide group-data-[state=checked]:text-primary-foreground">{option.label}</span>
                                                                {field.value === option.value && <CircleCheck className="size-5 text-primary fill-background transition-all ease-in-out" />}
                                                            </RadioGroupPrimitive.Item>
                                                        ))}
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem className="space-y-0">
                                                <FormControl>
                                                    <label className={cn(
                                                        "flex items-center justify-between p-4 rounded-xl border-2 cursor-pointer transition-all ease-in-out",
                                                        ...
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            )}><CircleCheck className="size-5" /></div>
                                                            <div>
                                                                <div className={cn("text-sm font-bold", field.value ? "text-primary" : "text-foreground")}>Set as default address</div>
                                                        ...
                                                        <Checkbox
                                                            ...
                                                            className="size-5 rounded-md border-2 border-muted-foreground/30 data-[state=checked]:border-primary data-[state=checked]:bg-primary"
                                                        />
                                                    </label>
                                                </FormControl>
                                                <FormMessage className="text-[10px]" />
                ...
                <DialogFooter className="px-6 py-4 bg-muted/30 border-t border-border gap-2">
                    ...
                    <Button type="submit" form="address-form" className="" disabled={!form.formState.isValid}>{submitLabel}</Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    );
